...
### 1순위: Post-MVP Unit 14 — 로그아웃 UI와 세션 종료 흐름

- 사유: Unit 12의 `clearSessionAtom`이 이미 준비되어 있고 외부 의존성이 없다.
- 핵심 결과: 앱 내부 헤더에서 로그아웃 가능, 세션 제거 후 `/login` 이동.
- 지시 문서: `docs/CURRENT_TASK.md`

### 2순위: Post-MVP Unit 15 — 수동 자산 persistence 전환
...
### 3순위: Post-MVP Unit 16 — 포트폴리오 종목별 계산 SSOT 이관
...
### 4순위: Post-MVP Unit 17 — MSW 브라우저 워커 준비
...
### 5순위: Post-MVP Unit 18 — 다크 테마/모바일 QA 보강
...
현재 다음 작업은 `Post-MVP Unit 14 — 로그아웃 UI와 세션 종료 흐름`이다.
